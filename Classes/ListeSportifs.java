...
public class ListeSportifs {
    ...
    public static boolean changeEtatSportif(String pseudo){
    	...
    	pseudo = pseudo.toUpperCase();
    	 while(!trouveOk || j<sportifs.size()){
             if(pseudo.equals(((Sportif) sportifs.get(j)).getPseudo())){
            	 trouveOk=true;
            	 boolean actif =  ((Sportif) sportifs.get(j)).getActif();
            	 ((Sportif) sportifs.get(j)).setActif(!actif);
            	 supprOk= true;
             }
             j++;
         }
    ...
    public static void main(String[] args) {
        ...
        /*TEST MODIF ETAT SPORTIF*/
        System.out.println("TEST CHANGEMENT ETAT");
        pseudo="pseudo29";
        changeEtatSportif(pseudo);
        afficheSportifs();
        System.out.println("FIN TEST ");
        System.out.println("--------------");
