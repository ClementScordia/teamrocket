...
public class ListeSportifs {
    private static ArrayList sportifs = new ArrayList<Sportif2>();
    ...
    public static boolean pseudoExistant(String pseudo){
        ...
        for(int i=0;i<sportifs.size();i++){
            if(pseudo.equals(((Sportif2) sportifs.get(i)).getPseudo())){
                pseudoValide = false;
            }
        }
        if(!pseudoValide){
    ...
    public static boolean sportifExistant(String nom, String prenom, Date date){
        ...
        for(int j=0;j<sportifs.size();j++){
            if(nom.equals(((Sportif2) sportifs.get(j)).getNom())){
                if(prenom.equals(((Sportif2) sportifs.get(j)).getPrenom())){
                    if(date.equals(((Sportif2) sportifs.get(j)).getDateNaissance())){
                        sportifValide = false;
                    }
                }
            }
    ...
    public static boolean ajoutSportif(String nom,String prenom,String pseudo,String sport,Date date){
        ...
        if(testNom && testPrenom && testPseudo && testExiste){
            Sportif2 sportif = new Sportif2(nom,prenom,pseudo,sport,date);
    ...
    public static boolean modifNom(String pseudo, String nouveauNom){
        ...
        while(!trouveOk || j<sportifs.size()){
            if(pseudo.equals(((Sportif2) sportifs.get(j)).getPseudo())){
                //on indique quon a trouver le pseudo
                trouveOk=true;
                //on recup les valeurs relativ au pseudo
                String ancienNom = ((Sportif2) sportifs.get(j)).getNom();
                String prenom = ((Sportif2) sportifs.get(j)).getPrenom();
                Date date = ((Sportif2) sportifs.get(j)).getDateNaissance();
                ...
                if(testExiste){
                    ((Sportif2) sportifs.get(j)).setNom(nouveauNom);
                    modifOk = true;
                }else{
    ...
    public static boolean modifPrenom(String pseudo, String nouveauPrenom){
        ...
        while(!trouveOk || j<sportifs.size()){
            if(pseudo.equals(((Sportif2) sportifs.get(j)).getPseudo())){
                ...
                trouveOk=true;
                //on recup les valeurs relativ au pseudo
                String nom = ((Sportif2) sportifs.get(j)).getNom();
                String ancienPrenom = ((Sportif2) sportifs.get(j)).getPrenom();
                Date date = ((Sportif2) sportifs.get(j)).getDateNaissance();
                ...
                //si elle n'existe pas on peut modifier le nom
                if(testExiste){
                    ((Sportif2) sportifs.get(j)).setPrenom(nouveauPrenom);
                    modifOk = true;
                }else{
    ...
    public static boolean modifDateNaissance(String pseudo, Date nouvelleDate){
        ...
        while(!trouveOk || j<sportifs.size()){
            if(pseudo.equals(((Sportif2) sportifs.get(j)).getPseudo())){
                ...
                String nom = ((Sportif2) sportifs.get(j)).getNom();
                String prenom = ((Sportif2) sportifs.get(j)).getPrenom();
                Date ancienneDate = ((Sportif2) sportifs.get(j)).getDateNaissance();
                
                //on test si la personne n'existe pas dans la base 
                testExiste = sportifExistant(nom, prenom, nouvelleDate);
                //si elle n'existe pas on peut modifier le nom
                if(testExiste){
                    ((Sportif2) sportifs.get(j)).setDateNaissance(nouvelleDate);
                    modifOk = true;
                }else{
    ...
    public static void main(String[] args) {
        ...
        ajoutSportif(nom,prenom,pseudo,"Foot",date);
        afficheSportifs();
